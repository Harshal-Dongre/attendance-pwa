...
//         }, 400); // 500ms holding window triggers details display
//       };

//       row.ontouchend = () => {
//         clearTimeout(holdTimer);
//         // Add a slight delay before closing so they can actually read it
//         setTimeout(() => { peekPanel.style.display = 'none'; }, 1000);
//       };
...
const ui = {
  ...
  showAttendanceView(lecIdx) {
    this.currentLecIdx = lecIdx;
    const lec = db.semesters[this.currentSemIdx].subjects[this.currentSubIdx].lectures[lecIdx];
    document.getElementById("appTitle").innerText = `Attendance: ${lec.date}`;
    document.getElementById("subjectTabs").style.display = "none";
    this.switchSubView('attendance');
  },
  ...
  renderSemesters() {
    const list = document.getElementById("semesterList");
    list.innerHTML = db.semesters.length === 0 ? "<p>No semesters found.</p>" : "";
    db.semesters.forEach((sem, i) => {
      const card = document.createElement("div");
      card.className = "card";
      card.innerHTML = `
        <h3>${sem.name}</h3>
        <p>${sem.subjects?.length || 0} Subjects</p>
        <button class="btn btn-action" onclick="ui.showSubjectView(${i})">Open Semester</button>
      `;
      list.appendChild(card);
    });
  },
  ...
  renderSubjects() {
    ...
    sem.subjects?.forEach((sub, i) => {
      ...
      card.innerHTML = `
        <h3>${sub.name}</h3>
        <p>${sub.students?.length || 0} Students | ${sub.lectures?.length || 0} Lectures</p>
        <button class="btn btn-action" onclick="ui.showLectureView(${i})">Manage Attendance</button>
      `;
      list.appendChild(card);
    });
  },
  ...
  renderLectures() {
    ...
    sub.lectures?.forEach((lec, i) => {
      ...
      item.className = "lecture-item";
      item.innerHTML = `
        <span><strong>${lec.date}</strong> <small>${lec.time}</small></span>
  ...
  renderAttendance() {
    ...
    lec.attendance.forEach((student, i) => {
      ...
      row.innerHTML = `
        <div class="short-id-trigger">${student.short}</div>
        <div class="student-peek-panel" id="peek-${i}">
          <span class="peek-name">${student.name}</span>
          <span class="peek-roll">Roll No: ${student.id}</span>
        </div>
      `;

      let holdTimer;
      const peek = row.querySelector(`#peek-${i}`);

      // TAP: Toggle
      row.onclick = () => actions.toggleAttendanceStatus(i);

      // LONG PRESS / HOVER: Show Name (Peek)
      const show = () => { peek.style.display = 'flex'; if(navigator.vibrate) navigator.vibrate(30); };
      const hide = () => { peek.style.display = 'none'; };

      row.onmouseenter = show;
      row.onmouseleave = hide;
      row.ontouchstart = (e) => { holdTimer = setTimeout(show, 400); };
      row.ontouchend = () => { clearTimeout(holdTimer); setTimeout(hide, 800); };
  ...
  renderRoster() {
    ...
    sub.students?.forEach(s => {
      const row = document.createElement("div");
      row.className = "roster-row";
      row.innerHTML = `<span>${s.short}</span> <span>${s.id}</span> <span>${s.name}</span>`;
      container.appendChild(row);
    });
  }
};
